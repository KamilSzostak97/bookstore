...
def home(request):
    ...
    if 'search' in request.GET:
        if request.GET['date_search'] is not None and request.GET['date_search'].isnumeric():
            books=Book.objects \
                .filter(published_date__gte=request.GET['search']) \
...
def import_books(request):
    ...
    if 'import' in request.GET:
        ...
        response_api = requests.get( \
                    ...
                    {filter_category}:{search}&printType=books&key={GOOGLE_API_KEY}')

        data = response_api.text
        parse_json = json.loads(data)

        if parse_json['totalItems']==0:
            context={'messege':'No matching entries found'}
            return render(request,'book/importbooks.html', context)

        for book_index in range(len(parse_json['items'])):
            model = Book()
            model.title = parse_json['items'][book_index]['volumeInfo']['title']
            authors = []
            for author_index in range(len(parse_json
                                ['items'][book_index]
                                ['volumeInfo']['authors'])):
                authors.append(parse_json['items'][book_index]
                                ['volumeInfo']['authors'][author_index])
            ...
            for isbn_index in range(len(parse_json['items'][book_index]
                                    ['volumeInfo']['industryIdentifiers'])):
                isbn.append(parse_json['items'][book_index]
                            ['volumeInfo']['industryIdentifiers'][isbn_index]['identifier'])
            ...
            if "imageLinks" in parse_json['items'][book_index]['volumeInfo']:
                model.cover_picture = parse_json['items'][book_index] \
...
def rest_api(request):
    ...
    if 'search' in request.GET:
        ...
        return redirect(
            f'http://127.0.0.1:8000/book/?search={search_value}&filterby={filter_category}')
